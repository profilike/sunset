...
function sunset_posted_meta(){
	...
	if(!empty($categories) ):
		foreach($categories as $category):
			...
			$output .= '<a href="' . esc_url(get_category_link($category->term_id)) . '" title="' . esc_attr(sprintf('View all posts in %s', $category->name)) . '">
			' . esc_html($category->name) . '</a>';
			$i++;
		endforeach;
	endif;

	return '<span class="posted-on">Posted <a href="'. esc_url(get_permalink()) .'">' . $posted_on . '</a> ago
	</span> / <span class="posted-in">' . $output . '</span>';
}
function sunset_posted_footer(){
	...
	return '<div class="post-footer-container">
				<div class="row">
					<div class="col-xs-12 col-sm-6">'. get_the_tag_list('<div class="tags-list"><span class="icon sunset-tag"></span>',' ','</div>') .'</div>		
...
function mailtrap($phpmailer) {
  ...
  $phpmailer->Port = 2525;
  $phpmailer->Username = '393f6388b84811';
  $phpmailer->Password = 'd05302b5c899ca';
}
